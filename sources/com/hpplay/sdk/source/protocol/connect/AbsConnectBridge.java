...
public abstract class AbsConnectBridge {
    ...
    private String getSinkKey() {
        ...
        if (lelinkServiceInfo != null) {
            str2 = String.valueOf(lelinkServiceInfo.getAppId());
            str = this.mServiceInfo.getUid();
        } else {
            str2 = "";
            str = str2;
        }
    ...
    public void callbackPass(PassBean passBean) {
        ...
        for (OnPassSendCompleteListener onPassReversed : this.onPassSendCompleteListenerMap.values()) {
            onPassReversed.onPassReversed(passBean);
        }
    }
    ...
    public void notifyPassReceivedData(int i10, BaseBean baseBean) {
        for (OnPassReceivedListener onPassReversed : this.onPassReceivedListenerMap.values()) {
            onPassReversed.onPassReversed(i10, baseBean);
        }
    }
        ...
        if (!TextUtils.isEmpty(str)) {
            try {
                for (String split : str.split(";")) {
                    ...
                    this.mPassSMMap.put(Integer.valueOf(split2[0]), Integer.valueOf(split2[1]));
                }
            } catch (Exception e10) {
