...
public class MirrorEventReceiver extends ProtocolCore implements Runnable {
    ...
    public String getMirrorEnventIP() {
        ...
        if (ProtocolUtils.isNetworkConnected(ModuleLinker.getInstance().getContext())) {
            str = ProtocolUtils.getWifiIp();
            CLog.i(TAG, "wifi   ip  " + str.replace(".", "") + "    LoaclIp  ");
            if (TextUtils.isEmpty(str)) {
                str = ProtocolUtils.getLoaclIp();
            }
        } else {
            CLog.i(TAG, "use moble host ip  ");
            str = ProtocolUtils.getLoaclIp();
        }
        CLog.i(TAG, "use realIp " + str);
        return str;
    }
    ...
    public void run() {
        boolean createMirrorEventServer = createMirrorEventServer((String) null, genMirrorEventPort());
        ...
        if (!createMirrorEventServer) {
            for (int i10 = 0; i10 < 5; i10++) {
                ...
                boolean createMirrorEventServer2 = createMirrorEventServer((String) null, nextInt);
