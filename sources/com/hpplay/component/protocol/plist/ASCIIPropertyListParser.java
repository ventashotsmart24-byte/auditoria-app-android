...
public final class ASCIIPropertyListParser {
    ...
    private NSObject parseData() {
        ...
        if (accept('*')) {
            ...
            if (accept((char) DATA_GSBOOL_BEGIN_TOKEN)) {
                ...
                if (accept((char) DATA_GSDATE_BEGIN_TOKEN)) {
                    skip();
                    nSNumber = new NSDate(readInputUntil((char) DATA_END_TOKEN));
                } else if (accept(DATA_GSINT_BEGIN_TOKEN, DATA_GSREAL_BEGIN_TOKEN)) {
                    skip();
                    nSNumber = new NSNumber(readInputUntil((char) DATA_END_TOKEN));
                } else {
                    nSObject = null;
                }
                nSObject = nSNumber;
        ...
        String replaceAll = readInputUntil((char) DATA_END_TOKEN).replaceAll("\\s+", "");
        ...
        while (true) {
            ...
            if (c10 != '\"' || (cArr[i10 - 1] == '\\' && z10)) {
                ...
                if (accept((char) QUOTEDSTRING_ESCAPE_TOKEN)) {
                    z10 = this.data[this.index - 1] != '\\' || !z10;
                }
                skip();
            } else {
                try {
    ...
    private void skipWhitespacesAndComments() {
        while (true) {
            if (accept(WHITESPACE_CARRIAGE_RETURN, 10, ' ', 9)) {
                skip();
            } else {
                boolean z10 = true;
                if (acceptSequence('/', '/')) {
    ...
    public static NSObject parse(File file, String str) {
        FileInputStream fileInputStream = new FileInputStream(file);
        try {
            return parse((InputStream) fileInputStream, str);
        } finally {
            try {
                fileInputStream.close();
            } catch (IOException unused) {
