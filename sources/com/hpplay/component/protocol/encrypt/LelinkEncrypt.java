...
public class LelinkEncrypt {
    ...
    private boolean chachaDecrypt(byte[] bArr, byte[] bArr2) {
        if (bArr != null && !this.isRelase) {
            ...
            if (Poly1305.crypto_onetimeauth_verify(bArr2, 0, bArr, 0, (long) bArr.length, bArr3) == 0) {
                return true;
            }
        }
        return false;
    ...
    private byte[] chachaEncryptAndGenSignature(byte[] bArr) {
        ...
        Poly1305.crypto_onetimeauth(bArr3, 0, bArr, 0, (long) bArr.length, bArr2);
    ...
    public byte[] decryptData(byte[] bArr) {
        int i10;
        if (bArr == null || bArr.length == 0 || this.isRelase || (i10 = (bArr[0] & UnsignedBytes.MAX_VALUE) | ((bArr[1] & UnsignedBytes.MAX_VALUE) << 8) | ((bArr[2] & UnsignedBytes.MAX_VALUE) << 16) | ((bArr[3] & UnsignedBytes.MAX_VALUE) << Ascii.CAN)) > 5120) {
            return null;
        }
        try {
    ...
    public byte[] genSha512(byte[] bArr, byte[] bArr2, int i10) {
        try {
            ...
            return Arrays.copyOfRange(instance.digest(), 0, i10);
    ...
    public byte[] genVerrifyM1Request() {
        ...
        if (i10 == 0) {
            ...
            Curve25519.keygen(bArr3, (byte[]) null, this.mCurvePrkey);
