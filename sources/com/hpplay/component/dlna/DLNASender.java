    ...
    private final Action.ResponseCallbackLisener responseCallbackLisener = new Action.ResponseCallbackLisener() {
        public void onCallback(String str, String str2) {
            try {
                ...
                CLog.w(DLNASender.TAG, (Throwable) e10);
            }
        }
    };
    ...
    public String getPositionInfo() {
        ...
        if (postControlAction) {
            ...
            if (TextUtils.isEmpty(argumentValue) || TextUtils.equals(argumentValue, NOT_IMPLEMENTED)) {
                j10 = formatToMillis(argumentValue2);
                CLog.i(TAG, "use reltime " + j10);
            } else if (TextUtils.isEmpty(argumentValue2) || TextUtils.equals(argumentValue2, NOT_IMPLEMENTED)) {
                j10 = formatToMillis(argumentValue);
            } else {
                j10 = formatToMillis(argumentValue2);
                long formatToMillis = formatToMillis(argumentValue);
                if (formatToMillis > 0 && !TextUtils.equals(argumentValue, argumentValue4)) {
                    j10 = formatToMillis;
                } else if (j10 <= 0) {
                    j10 = 0;
                }
            }
            CLog.i(TAG, "dlna call back time : position :   " + j10 + " TrackDuration:  " + argumentValue4);
            long formatToMillis2 = formatToMillis(argumentValue4);
            JSONObject jSONObject = new JSONObject();
            if (formatToMillis2 > -1) {
                try {
                    formatToMillis2 /= 1000;
                } catch (Exception e10) {
                    CLog.w(TAG, (Throwable) e10);
                }
            }
            ...
            if (j10 > 0 && (i10 = this.mStartPosition) > 0) {
                seek(String.valueOf(i10 * 1000));
                this.mStartPosition = 0;
            }
            jSONObject.put("position", j10);
            jSONObject.put("duration", formatToMillis2);
