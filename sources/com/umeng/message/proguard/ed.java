...
public final class ed {
    ...
    public static float a(Point point) {
        try {
            DisplayMetrics displayMetrics = Resources.getSystem().getDisplayMetrics();
            return ((float) Math.round(Math.sqrt(Math.pow((double) (((float) point.x) / displayMetrics.xdpi), 2.0d) + Math.pow((double) (((float) point.y) / displayMetrics.ydpi), 2.0d)) * 100.0d)) / 100.0f;
        } catch (Throwable unused) {
            return 0.0f;
        }
    }
