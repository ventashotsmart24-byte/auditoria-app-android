...
public abstract class BaseModalLayout extends FrameLayout {
    ...
    public int dpToPixels(int i10) {
        return (int) Math.floor((double) TypedValue.applyDimension(1, (float) i10, this.mDisplay));
    }
    ...
    public void onMeasure(int i10, int i11) {
        ...
        for (int i12 = 0; i12 < getChildCount(); i12++) {
            ...
            if (childAt.getVisibility() != 8) {
                ...
                Logging.logdNumber("Skipping GONE child", (float) i12);
            }
        }
    }

    public int roundToNearest(int i10, int i11) {
        return i11 * Math.round(((float) i10) / ((float) i11));
    }

    public void layoutChild(View view, int i10, int i11, int i12, int i13) {
        Logging.logdPair("\tleft, right", (float) i10, (float) i12);
        Logging.logdPair("\ttop, bottom", (float) i11, (float) i13);
