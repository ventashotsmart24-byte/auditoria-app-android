...
public final class BloomFilter<T> implements Predicate<T>, Serializable {
    ...
    public static <T> BloomFilter<T> create(Funnel<? super T> funnel2, int i10, double d10) {
        return create(funnel2, (long) i10, d10);
    }
    ...
    public static <T> BloomFilter<T> readFrom(InputStream inputStream, Funnel<? super T> funnel2) {
        ...
        try {
            ...
            try {
                ...
                return new BloomFilter<>(lockFreeBitArray, i11, funnel2, bloomFilterStrategies);
    ...
    public long approximateElementCount() {
        ...
        double d10 = (double) bitSize;
        Double.isNaN(bitCount);
        Double.isNaN(d10);
        Double.isNaN(d10);
        double d11 = (-Math.log1p(-(bitCount / d10))) * d10;
        double d12 = (double) this.numHashFunctions;
        Double.isNaN(d12);
    ...
    private BloomFilter(BloomFilterStrategies.LockFreeBitArray lockFreeBitArray, int i10, Funnel<? super T> funnel2, Strategy strategy2) {
        boolean z10 = true;
        Preconditions.checkArgument(i10 > 0, "numHashFunctions (%s) must be > 0", i10);
        Preconditions.checkArgument(i10 > 255 ? false : z10, "numHashFunctions (%s) must be <= 255", i10);
    ...
    public static <T> BloomFilter<T> create(Funnel<? super T> funnel2, long j10, double d10) {
        return create(funnel2, j10, d10, BloomFilterStrategies.MURMUR128_MITZ_64);
    }

    @VisibleForTesting
    public static <T> BloomFilter<T> create(Funnel<? super T> funnel2, long j10, double d10, Strategy strategy2) {
        Preconditions.checkNotNull(funnel2);
        boolean z10 = true;
        Preconditions.checkArgument(j10 >= 0, "Expected insertions (%s) must be >= 0", j10);
        Preconditions.checkArgument(d10 > 0.0d, "False positive probability (%s) must be > 0.0", (Object) Double.valueOf(d10));
        if (d10 >= 1.0d) {
            z10 = false;
        }
        Preconditions.checkArgument(z10, "False positive probability (%s) must be < 1.0", (Object) Double.valueOf(d10));
        ...
        try {
            return new BloomFilter<>(new BloomFilterStrategies.LockFreeBitArray(optimalNumOfBits), optimalNumOfHashFunctions(j10, optimalNumOfBits), funnel2, strategy2);
    ...
    public static <T> BloomFilter<T> create(Funnel<? super T> funnel2, int i10) {
        return create(funnel2, (long) i10);
    }

    public static <T> BloomFilter<T> create(Funnel<? super T> funnel2, long j10) {
        return create(funnel2, j10, 0.03d);
    }
}
