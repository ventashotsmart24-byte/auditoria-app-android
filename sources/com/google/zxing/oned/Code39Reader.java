    ...
    private static int[] findAsteriskPattern(BitArray bitArray, int[] iArr) {
        ...
        while (nextSet < size) {
            if (bitArray.get(nextSet) ^ z10) {
                ...
                if (i11 != i12) {
                    i11++;
                } else if (toNarrowWidePattern(iArr) == ASTERISK_ENCODING && bitArray.isRange(Math.max(0, i10 - ((nextSet - i10) / 2)), i10, false)) {
                ...
                if (patternToChar == '*') {
                    ...
                    if (nextSet2 == size || (i15 << 1) >= i13) {
                        ...
                        if (sb.length() != 0) {
                            ...
                            float f10 = (float) i10;
                            return new Result(str, (byte[]) null, new ResultPoint[]{new ResultPoint(((float) (findAsteriskPattern[1] + findAsteriskPattern[0])) / 2.0f, f10), new ResultPoint(((float) nextSet) + (((float) i13) / 2.0f), f10)}, BarcodeFormat.CODE_39);
                        }
                        throw NotFoundException.getNotFoundInstance();
                    }
                    throw NotFoundException.getNotFoundInstance();
