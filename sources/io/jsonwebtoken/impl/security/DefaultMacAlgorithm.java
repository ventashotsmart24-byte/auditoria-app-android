...
final class DefaultMacAlgorithm extends AbstractSecureDigestAlgorithm<SecretKey, SecretKey> implements MacAlgorithm {
    ...
    private void assertAlgorithmName(SecretKey secretKey, boolean z10) {
        ...
        } else if (!KeysBridge.isGenericSecret(secretKey) && isJwaStandard() && !isJwaStandardJcaName(algorithm)) {
    ...
    public static DefaultMacAlgorithm findByKey(Key key) {
        DefaultMacAlgorithm defaultMacAlgorithm;
        String findAlgorithm = KeysBridge.findAlgorithm(key);
        if (Strings.hasText(findAlgorithm) && (defaultMacAlgorithm = JCA_NAME_MAP.get(findAlgorithm.toUpperCase(Locale.ENGLISH))) != null && Bytes.bitLength(KeysBridge.findEncoded(key)) >= ((long) defaultMacAlgorithm.getKeyBitLength())) {
    ...
    public byte[] doDigest(final SecureRequest<InputStream, SecretKey> secureRequest) {
        return (byte[]) jca(secureRequest).withMac(new CheckedFunction<Mac, byte[]>() {
    ...
    public boolean doVerify(VerifySecureDigestRequest<SecretKey> verifySecureDigestRequest) {
        ...
        return MessageDigest.isEqual(digest, digest(verifySecureDigestRequest));
    }
    ...
    public void validateKey(Key key, boolean z10) {
        ...
        } else if (!(key instanceof Password)) {
