...
final class EcSignatureAlgorithm extends AbstractSignatureAlgorithm {
    ...
    public final int signatureByteLength;
    ...
    public static byte[] transcodeDERToConcat(byte[] bArr, int i10) {
        ...
        byte b13 = bArr[i11 - 1];
        if ((b13 & UnsignedBytes.MAX_VALUE) == bArr.length - i11 && (b13 & UnsignedBytes.MAX_VALUE) == b11 + 2 + 2 + b12 && bArr[i11] == 2 && bArr[i13] == 2) {
            int i15 = max * 2;
        ...
        return ((Boolean) jca(verifySecureDigestRequest).withSignature(new CheckedFunction<Signature, Boolean>() {
            public Boolean apply(Signature signature) {
                ...
                try {
                    ...
                    return Boolean.valueOf(EcSignatureAlgorithm.this.verify(signature, (InputStream) verifySecureDigestRequest.getPayload(), digest));
                } catch (Exception e10) {
    ...
    public KeyPairBuilder keyPair() {
        return (KeyPairBuilder) new DefaultKeyPairBuilder("EC", (AlgorithmParameterSpec) this.KEY_PAIR_GEN_PARAMS).random(Randoms.secureRandom());
    }
    ...
    public void validateKey(Key key, boolean z10) {
        ...
        if (KEY_ALG_NAMES.contains(KeysBridge.findAlgorithm(key))) {
            ...
            if (findBitLength >= 0 && Bytes.length(findBitLength) * 2 != this.signatureByteLength) {
                throw new InvalidKeyException("The provided Elliptic Curve " + AbstractSecureDigestAlgorithm.keyType(z10) + " key size (aka order bit length) is " + Bytes.bitsMsg((long) findBitLength) + ", but the '" + getId() + "' algorithm requires EC Keys with " + Bytes.bitsMsg((long) this.orderBitLength) + " per [RFC 7518, Section 3.4](https://www.rfc-editor.org/rfc/rfc7518.html#section-3.4).");
            }
            return;
        }
