...
public abstract class AbstractCompressionAlgorithm implements CompressionAlgorithm, CompressionCodec {
    private final Function<byte[], byte[]> COMPRESS_FN = forCompression(new CheckedFunction<byte[], byte[]>() {
        public byte[] apply(byte[] bArr) {
            return AbstractCompressionAlgorithm.this.doCompress(bArr);
        }
    });
    private final Function<byte[], byte[]> DECOMPRESS_FN = forDecompression(new CheckedFunction<byte[], byte[]>() {
        public byte[] apply(byte[] bArr) {
            return AbstractCompressionAlgorithm.this.doDecompress(bArr);
        }
    });
    private final Function<InputStream, InputStream> IS_WRAP_FN = forDecompression(new CheckedFunction<InputStream, InputStream>() {
        public InputStream apply(InputStream inputStream) {
            return AbstractCompressionAlgorithm.this.doDecompress(inputStream);
        }
    });
    private final Function<OutputStream, OutputStream> OS_WRAP_FN = forCompression(new CheckedFunction<OutputStream, OutputStream>() {
    ...
    public byte[] doCompress(byte[] bArr) {
        ...
        OutputStream compress = compress((OutputStream) byteArrayOutputStream);
        try {
            compress.write(bArr);
            compress.flush();
    ...
    public final byte[] compress(byte[] bArr) {
        ...
        return this.COMPRESS_FN.apply(bArr);
    }
    ...
    public final byte[] decompress(byte[] bArr) {
        ...
        return this.DECOMPRESS_FN.apply(bArr);
    }
}
