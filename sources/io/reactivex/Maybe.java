...
public abstract class Maybe<T> implements MaybeSource<T> {
    ...
    public static Maybe<Long> timer(long j10, TimeUnit timeUnit, Scheduler scheduler) {
        ...
        ObjectHelper.requireNonNull(scheduler, "scheduler is null");
        return RxJavaPlugins.onAssembly(new MaybeTimer(Math.max(0, j10), timeUnit, scheduler));
    }
    ...
    public final Maybe<T> delay(long j10, TimeUnit timeUnit, Scheduler scheduler) {
        ObjectHelper.requireNonNull(timeUnit, "unit is null");
        ObjectHelper.requireNonNull(scheduler, "scheduler is null");
        return RxJavaPlugins.onAssembly(new MaybeDelay(this, Math.max(0, j10), timeUnit, scheduler));
    }
    ...
    public final T blockingGet(T t10) {
        ...
        BlockingMultiObserver blockingMultiObserver = new BlockingMultiObserver();
        subscribe(blockingMultiObserver);
        return blockingMultiObserver.blockingGet(t10);
    }
    ...
    public final <U> Maybe<T> timeout(b bVar) {
        ...
        return RxJavaPlugins.onAssembly(new MaybeTimeoutPublisher(this, bVar, (MaybeSource) null));
    }
